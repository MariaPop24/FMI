...
2) class frequency_queue: stores a multiset of elements. Upon request a least/most repeated element can be returned and removed from the structure.
3) class partition_refinement: implements the basic partition refinement strategy, and Hopcroft rule for LexDFS.
@author: G. Ducoffe
*/
...
class frequency_queue {
   ...
   public:
	inline bool empty(void) const { return _q.empty(); }
	inline unsigned int size(void) const { return _q.size(); }
	unsigned int count(unsigned int) const; //number of repetitions of an element
	void insert(unsigned int); //insertition of a new element
	void remove_once(unsigned int); //remove at most one occurence
	unsigned int peek_min(void) const; //return a least repeated element
	unsigned int peek_max(void) const; //return a most repeated element
	void erase_min(void); //removes a least repeated element
	void erase_max(void); //removes a most repeated element
};

/*----------------------------------- Partition Refinement ------------------------------------------------------------------*/

typedef priority_queue<unsigned int, vector<unsigned int>, greater<unsigned int> > min_heap;
...
class partition_refinement  {
	...
	private:
  	...
  	public:
		...
		bool make_next(unsigned int); //checks whether an element is in the last set (and if true, removes it)

		void refine(const unordered_set<unsigned int>&); //used for LexBFS
		void hopcroft_refine(const unordered_set<unsigned int>&); //used for LexDFS
		void rrefine(const unordered_set<unsigned int>&); //used for LexBFS in the complement

		list< list<unsigned int> > get_partition(void) const;
};

ostream& operator<<(ostream& os, const partition_refinement&);
